...
try:
    ...
    # DO NOT kill gphoto2 here; that can interrupt in-flight captures.
...
try:
    # Avoid killing gphoto2 here; we just want to read the config.
    result = subprocess.run(
    ...
    def capture_image(self, retry=True):
        ...
        try:
            # Ensure exclusive access to gphoto2 during capture
...
# Some cameras may still fire the shutter but gphoto2 returns non-zero due to transient PTP warnings.
success_code = (result.returncode == 0)
non_fatal = False
if not success_code and result.stderr:
    low = result.stderr.lower()
    # Treat these as non-fatal if the camera likely fired
    non_fatal = any(k in low for k in [
        "ptp i/o error", "device busy", "resource busy", "usb device reset"
    ])
