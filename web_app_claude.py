...
class FilmScanner:
    ...
    def check_camera(self):
        """Check camera connection - with better state management"""
        # Reduced cache time from 5 seconds to 2 seconds for more responsive checking
        current_time = time.time()
        if current_time - self.last_camera_check < 2:  # Reduced from 5
        ...
        try:
            # More thorough cleanup before checking
            self._kill_gphoto2()
            ...
            if result.returncode == 0 and "usb" in result.stdout.lower():
                ...
                for line in lines[2:]:
                    if 'usb' in line.lower():
                        ...
                        # Clean up after detection to leave camera in good state
                        self._kill_gphoto2()
                        return True
            ...
            print(f"✗ {self.camera_error}")
            self._kill_gphoto2()  # Clean up on timeout
            return False
            ...
            self._kill_gphoto2()  # Clean up on error
            return False
    
    def _kill_gphoto2(self):
        """Thoroughly kill any gphoto2 processes and wait for USB release"""
        try:
            # First, try to reset the camera USB connection if possible
            try:
            ...
            # Total wait: 1.4 seconds for USB to fully release (increased from 1.0)
            print("   ✓ Cleaned up gphoto2 processes")
        except Exception as e:
    ...
    def check_viewfinder_state(self):
        ...
        try:
            ...
            if result.returncode == 0:
                # Parse the output to see current value
                for line in result.stdout.split('\n'):
                    if 'Current:' in line:
                        if '1' in line or 'On' in line or 'True' in line:
                            return True
                        else:
                            return False
            return False
    ...
    def enable_viewfinder(self):
        """Enable camera viewfinder for live preview"""
        try:
    ...
    def capture_image(self, retry=True):
        """Capture image to camera SD card - improved error handling and recovery"""
        try:
            print("📷 Capturing image...")
            
            # MUST kill any existing gphoto2 processes first (prevents -9 errors)
            self._kill_gphoto2()
            time.sleep(1.5)  # Increased from 1.0 - Give USB more time to release
            
            # Try to reset the camera connection before capture
            print("   Ensuring camera is ready...")
            ...
            if "usb" not in reset_result.stdout.lower():
                print("   ⚠ Camera not detected, waiting longer...")
                time.sleep(2)
                # Try one more time
                reset_result = subprocess.run(
                    ["gphoto2", "--auto-detect"],
            ...
            # Run capture with more explicit error handling
            result = subprocess.run(
            ...
            if result.stdout:
                ...
                # Check for success indicators in stdout even if return code is non-zero
                if any(indicator in stdout_lower for indicator in 
                       ["new file", "saving file", "captured", "downloading"]):
                    print("   ✓ Capture indicators found in output")
                    # Treat as success even if return code is non-zero
                    self.frame_count += 1
            ...
            # Check if capture succeeded based on return code
            if result.returncode == 0:
                # Success!
                self.frame_count += 1
                self.frames_in_strip += 1
                ...
                if result.stderr:
                    ...
                    if retry and any(word in error_msg for word in 
                                    ["claim", "busy", "lock", "resource", "device or resource busy"]):
                        print("   → Camera locked by another process, cleaning up and retrying...")
                        self._kill_gphoto2()
                        
                        # More aggressive cleanup - try to reset USB
                        try:
                            subprocess.run(["gphoto2", "--reset"], 
                    ...
                    if "ptp" in error_msg or "not found" in error_msg:
                        print("   → Check: Camera USB mode is PTP, camera is on, cable connected")
                    elif "card" in error_msg or "space" in error_msg:
                        print("   → Check: SD card is inserted and has free space")
                    elif "could not lock" in error_msg:
                        print("   → Camera is locked. Resetting...")
                        # Try a camera reset
                        subprocess.run(["gphoto2", "--reset"], 
                                     capture_output=True, timeout=3)
                        time.sleep(2)
    ...
    def debug_camera_state(self):
        """Debug function to check camera state and processes"""
        print("\n" + "="*60)
        print("CAMERA STATE DEBUG")
        print("="*60)
        
        # Check for gphoto2 processes
        try:
            result = subprocess.run(["ps", "aux"], capture_output=True, text=True)
            gphoto_processes = [line for line in result.stdout.split('\n') if 'gphoto' in line]
            if gphoto_processes:
                print("⚠ Found gphoto2 processes:")
                for proc in gphoto_processes:
                    print(f"   {proc}")
            else:
                print("✓ No gphoto2 processes running")
        except:
            pass
        ...
        try:
            ...
            print("\nUSB Devices:")
            for line in result.stdout.split('\n'):
                if 'canon' in line.lower() or 'nikon' in line.lower() or 'sony' in line.lower():
                    print(f"   📷 {line}")
        except:
            pass
        ...
        try:
            ...
            if result.returncode == 0:
                print("✓ Camera is responding to gphoto2")
                # Print first few lines of summary
                lines = result.stdout.split('\n')[:5]
                for line in lines:
                    print(f"   {line}")
            else:
                print("✗ Camera not responding to gphoto2")
        except:
            print("✗ gphoto2 --summary failed")
...
def new_strip():
    ...
    if action == 'capture_first':
        # Capture first frame of new strip
        if not scanner.capture_image():
            return jsonify({'success': False, 'message': 'Capture failed'})
        ...
        return jsonify({'success': True, 'strip': scanner.strip_count})
    
    # Just reset counters for new strip
    scanner.frames_in_strip = 0
    scanner.status_msg = "Ready for new strip"
    scanner.broadcast_status()
...
def live_preview():
    ...
    try:
        # Clean up any existing processes FIRST (critical!)
        scanner._kill_gphoto2()
        time.sleep(1.0)  # Give USB time to fully release
        
        # Enable viewfinder if not already enabled
        if not scanner.viewfinder_enabled:
            print("   Enabling viewfinder mode...")
            if not scanner.enable_viewfinder():
                print("✗ Could not enable viewfinder")
                scanner.status_msg = "✗ Viewfinder failed"
                scanner.broadcast_status()
                return jsonify({
                    'success': False,
                    'message': 'Could not enable camera viewfinder. Try manual focus.'
                })
        
        # Change to temp directory to capture preview
        os.chdir(temp_dir)
        ...
        if result.returncode != 0:
            ...
            if "viewfinder" in error_msg.lower():
                print("   → Camera doesn't support viewfinder mode")
            elif "claim" in error_msg.lower() or "lock" in error_msg.lower():
            ...
            return jsonify({
                'success': False,
                'message': f'Preview capture failed: {error_msg[:100]}'
            })
        
        # Find the preview file
        print("   Looking for preview file...")
